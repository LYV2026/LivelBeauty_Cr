...
export default function FeaturedProducts() {
  ...
  return (
    <section id="destacados" className="py-24 bg-brand-surface">
      <div className="container mx-auto px-4 md:px-6">
        <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-6">
          <div className="space-y-4">
            ...
            <h2 className="text-4xl md:text-5xl font-serif text-[#2c1810]">Productos <span className="italic text-brand-taupe">Destacados</span></h2>
          </div>
          <motion.a
            whileHover={{ x: 5 }}
            href="https://wa.me/50672952666"
            className="text-[#2c1810] font-bold uppercase tracking-widest text-sm border-b-2 border-brand-taupe pb-1"
          >
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
          {products.map((product, index) => (
            <motion.div
              ...
              className="group bg-[#fffaf7] rounded-[32px] overflow-hidden shadow-sm hover:shadow-xl hover:shadow-brand-taupe/15 transition-all border border-[#e8d5c8]"
            >
              ...
              <div className="p-6 space-y-3">
                <div className="flex items-center gap-1 text-brand-taupe">
                  {[...Array(5)].map((_, i) => <Star key={i} className="w-3 h-3 fill-current" />)}
                </div>
                <h3 className="text-lg font-serif font-bold text-[#2c1810]">{product.name}</h3>
                <p className="text-[#6b4c3b] text-sm line-clamp-2">{product.desc}</p>
                <div className="flex items-center justify-between pt-4">
                  <span className="text-xl font-serif font-bold text-brand-taupe">{product.price}</span>
                  <motion.a
                    ...
                    className="w-10 h-10 rounded-full bg-brand-taupe text-white flex items-center justify-center hover:bg-brand-rose transition-colors"
                  >
